 ...
 * セット間の休憩時間(秒)の選択肢。
 *
 * 【なぜ種目ごとに変えるか】これまで全種目一律90秒だった。
 * レッグプレスのような大きな複合種目と、二頭のカールのような単関節種目で
 * 同じ90秒というのは実態と合わない。前者はもっと要り、後者は短くて足りる。
 * タイマーが実態と合っていないと、鳴っても従わなくなり、
 * 結果としてタイマーそのものが無視される。
 *
 * 60〜180秒: 筋肥大目的のレジスタンストレーニングで一般に推奨される休憩の幅。
 * 【出典なし・実務上の仮定】この4段階という区切り自体は、
 * 60〜120秒程度という一般的な目安(js/energy.js の MINUTES_PER_SET のコメント参照)に、
 * 大きな複合種目向けの180秒を足して組み立てたもので、
 * 特定の研究が「この4つ」と述べているわけではない。
 */
export const REST_STEPS = [60, 90, 120, 180];

/** 全種目の既定。利用者が種目ごとに変更するまでこの値を使う */
export const DEFAULT_REST_SECONDS = 90;

/**
 * 次の休憩時間へ巡回する。末尾の次は先頭に戻る。
 * 一覧に無い値・不正な値は既定(90秒)へ寄せる
 * (「壊れていたら既定に戻る」方が、巡回の途中に飛ぶより予測しやすい)。
 */
export function nextRestSeconds(current) {
  const i = REST_STEPS.indexOf(current);
  if (i === -1) return DEFAULT_REST_SECONDS;
  return REST_STEPS[(i + 1) % REST_STEPS.length];
}
